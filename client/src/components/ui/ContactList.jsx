...
function ContactList({ contact, isChannel = false }) {
    ...
    <div className='mt-5 '>
      {
        contact.map((contact) => (
          ...
          >
            <div
              className=' flex gap-5 items-center justify-start text-neutral-300' >
              {
                !isChannel &&
                (<Avatar className="h-10 w-10 rounded-full overflow-hidden">
                  {contact.image ? (
                    ...
                    ${selectedChatData && selectedChatData._id === contact._id
                        ...
                        : getColor(contact.color)} uppercase h-10 w-10 text-lg border-[1px] flex justify-center items-center rounded-full `} >
                      {
                        contact.firstName
